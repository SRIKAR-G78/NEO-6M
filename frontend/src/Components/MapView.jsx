...
const MapView = ({ latitude, longitude }) => {
  ...
  // Update marker position when props change without panning the map
  useEffect(() => {
    const lat = parseFloat(latitude) || 0;
    const lon = parseFloat(longitude) || 0;
    ...
    // On first valid fix (non-zero coords), set the map view once so the map is centered initially
    if (!firstLocationSetRef.current && Math.abs(lat) > 0.000001 && Math.abs(lon) > 0.000001) {
      try {
