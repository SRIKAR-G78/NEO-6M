...
const KalmanView = ({ gpsData, onFilteredUpdate, refLat: propRefLat, refLon: propRefLon, neoLat: propNeoLat, neoLon: propNeoLon, manualComputeTrigger }) => {
  ...
  const [lastErrors, setLastErrors] = useState({ raw: 0, filtered: 0 });
  ...
  useEffect(() => {
    ...
    if (kf) {
      ...
      // compute distance errors to reference if provided via props
      const rlat = parseFloat(propRefLat ?? '');
      const rlon = parseFloat(propRefLon ?? '');
      let rawErr = null, filtErr = null;
      if (isFinite(rlat) && isFinite(rlon)) {
        rawErr = haversine(lat, lon, rlat, rlon);
        filtErr = haversine(flat, flon, rlat, rlon);
        setLastErrors({ raw: rawErr, filtered: filtErr });
      ...
      if (typeof onFilteredUpdate === 'function') {
        onFilteredUpdate({ lat: flat, lon: flon, rawError: rawErr, filteredError: filtErr, source: 'live' });
      }
    }
  }, [gpsData.latitude, gpsData.longitude, propRefLat, propRefLon]);
  ...
  useEffect(() => {
    if (manualComputeTrigger == null) return;
    const nlat = parseFloat(propNeoLat);
    const nlon = parseFloat(propNeoLon);
    const rlat = parseFloat(propRefLat);
    const rlon = parseFloat(propRefLon);
    if (!isFinite(nlat) || !isFinite(nlon) || !isFinite(rlat) || !isFinite(rlon)) return;

    const rawErr = haversine(nlat, nlon, rlat, rlon);
    const latestFiltered = filteredPoints[filteredPoints.length-1];
    const filtErr = latestFiltered ? haversine(latestFiltered.lat, latestFiltered.lon, rlat, rlon) : 0;
    const neoKalman = latestFiltered ? haversine(nlat, nlon, latestFiltered.lat, latestFiltered.lon) : 0;

    setLastErrors({ raw: rawErr, filtered: filtErr, neoKalman });
    setRawErrorSeries(prev => [...prev.slice(-399), rawErr]);
    setFilteredErrorSeries(prev => [...prev.slice(-399), filtErr]);
    ...
    if (typeof onFilteredUpdate === 'function') {
      onFilteredUpdate({ lat: latestFiltered ? latestFiltered.lat : null, lon: latestFiltered ? latestFiltered.lon : null, rawError: rawErr, filteredError: filtErr, neoKalman, source: 'manual' });
    }
  }, [manualComputeTrigger]);
        ...
        {(
          <div className="error-results-container">
            ...
            <div className="error-row">
              <span className="error-label">Raw Neo-6M → Ref:</span>
              <span className="error-value raw">{(lastErrors.raw || 0).toFixed(2)} m</span>
            </div>
            <div className="error-row">
              <span className="error-label">Kalman → Ref:</span>
              <span className="error-value filtered">{(lastErrors.filtered || 0).toFixed(2)} m</span>
            </div>
            <div className="error-row">
            ...
            <div className="error-comparison">
              <div className="comparison-bar">
                <div className="bar raw-bar" style={{width: `${Math.min(100, ((lastErrors.raw||0) / Math.max(lastErrors.raw||0, lastErrors.filtered||0, lastErrors.neoKalman||0, 0.01)) * 100)}%`}}></div>
              </div>
              <div className="comparison-bar">
                <div className="bar filtered-bar" style={{width: `${Math.min(100, ((lastErrors.filtered||0) / Math.max(lastErrors.raw||0, lastErrors.filtered||0, lastErrors.neoKalman||0, 0.01)) * 100)}%`}}></div>
              </div>
              <div className="comparison-bar">
                <div className="bar" style={{background:'linear-gradient(90deg,#f59e0b,#f97316)', width: `${Math.min(100, ((lastErrors.neoKalman||0) / Math.max(lastErrors.raw||0, lastErrors.filtered||0, lastErrors.neoKalman||0, 0.01)) * 100)}%`}}></div>
              </div>
            </div>
          </div>
        )}
